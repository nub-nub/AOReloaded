...
- **Both mouse buttons → forward**: managed by OnMouseDown/EndDrag detours. Entering BOTH_HELD dispatches `N3Msg_MovementChanged(StartForward)`. Releasing one button transitions to the remaining button's solo mode and dispatches `StopForward`. Continuous char-align during BOTH_HELD handled in CalcSteering.
