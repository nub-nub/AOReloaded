...
- **Enhanced timer bars** (`src/hooks/timer_bar_drag.cpp`): hooks both `CreateTimer` overloads (GUI.dll RVAs `0x518f0` and `0x5195b`) and `N3Msg_CastNanoSpell` (Interfaces.dll, resolved via `GetProcAddress`). Features: (1) **Dragging** — registers a `MouseEventFilter` with the input handler; during LMB drag, walks the game's `std::list<TimerBarBase_c*>` at `TimerSystemModule_t+0x28` and calls `RenderWindow_t::Reposition` on each bar. Position stored as absolute coords via `AOR_TBarX`/`AOR_TBarY` DValues. (2) **Scaling** — calls `RenderWindow_t::SetScale` + `Resize` to scale bars to custom dimensions via `AOR_TBarW`/`AOR_TBarH`. The `SetScale` alone doesn't rebuild GPU quads — must follow with `Resize` to trigger `RenderSprite_t::Resize` which rebuilds the textured quad vertices. (3) **Nano name display** — `CastNanoSpell` hook captures the nano's `Identity_t` (type `0xCF1B`, instance = AOID). On timer bar creation, `N3Msg_GetName` (GUI.dll cached ptr at `0x1a7724`) resolves the identity to the nano program name, replacing the generic "Nano program" label.
