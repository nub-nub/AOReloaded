...
- **Draggable timer bars** (`src/hooks/timer_bar_drag.cpp`): hooks `TimerSystemModule_t::CreateTimer` (GUI.dll RVA `0x518f0`, SEH prologue) to reposition newly created bars using a saved pixel offset. Registers a `MouseEventFilter` with the input handler to intercept LMB click-drag on any active timer bar. During drag, walks the game's `std::list<TimerBarBase_c*>` at `TimerSystemModule_t+0x28` and calls `Sprite_t::Reposition` on each bar's `RenderWindow_t`. Offset persisted via `AOR_TBarX`/`AOR_TBarY` DValues (written to .ini by the settings hook). Architecture designed for future extension: `RenderWindow_t::Resize` (RVA `0x20938`) is identified for bar resizing; `TextLine_t` at `TimerBarBase_c+0x0C` is the text overlay point for showing nano names.
